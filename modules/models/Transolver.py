...
class Transolver_2D(nn.Module):
    def __init__(self,
        ...
        super(Transolver_2D, self).__init__()
    ...
    def forward(self, fx, x=None):
        # fx in shape (B, H, W, C)
        fx = rearrange(fx, 'b h w c -> b (h w) c')

        if self.unified_pos or x is None:
            x = self.pos.repeat(fx.shape[0], 1, 1, 1).reshape(fx.shape[0], self.H * self.W, self.ref * self.ref).to(fx.device)
        if fx is not None:
            fx = torch.cat((x, fx), -1)
        ...
        fx = rearrange(fx, 'b (h w) c -> b h w c', h=self.H, w=self.W)
